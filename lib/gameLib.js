    ...
    if (data.rotasi == 0) {
      if (data.skalaX == 1 && data.skalaY == 1) {
        konten.drawImage(
          ...
          data.lebar,
          data.tinggi,
          data.x - (data.lebar * game.skalaSprite) / 2,
          data.y - (data.tinggi * game.skalaSprite) / 2,
        ...
        konten.drawImage(
          ...
          data.lebar,
          data.tinggi,
          -(data.lebar * game.skalaSprite) / 2,
          -(data.tinggi * game.skalaSprite) / 2,
          data.lebar * game.skalaSprite,
          data.tinggi * game.skalaSprite
        );
        konten.restore();
...
untuk membuat peta gunakan map editor
//------------------------------------------------------------*/
          ...
          if (item_type != undefined) {
            if (game.platformItems["item_" + item_type] != undefined) {
              ...
              if (
                ...
                game.map[tx][ty][0] = 0;
                this.mainkanSuara(dataSuara.coin);
              }
            }
          }
        }
...
function heroDead() {
  this.mainkanSuara(dataSuara.dead);
  game.aktif = false;
...
function gerakLevel(ob, sx, sy, audio) {
  var ox, oy, cx, cy;
  if (game.aktif) {
    if (sx > 0) {
      //kanan
    ...
    if (sy < 0 && !game.lompat) {
      game.lompat = true;
      game.lompatY = sy;
      this.mainkanSuara(audio);
    }
    
  }
}

function setPlatformEnemy(id, ob, width, height) {
  //loop cari posisi musuh tile id = 5
  for (var i = 0; i < game.map.length; i++) {
    for (var j = 0; j < game.map[0].length; j++) {
      if (game.map[i][j][0] == 5 && game.map[i][j][3] == id) {
        var musuh = setSprite(ob.animDiam, width, height);
        musuh.dataGambar = ob;
        musuh.charX = i;
        musuh.charY = j;
        musuh.posX = 0;
  ...
  for (var m = 0; m < game.platformEnemies.length; m++) {
    ...
    if (musuh.stat == 1) {
      ...
      if (musuh.sx > 0) {
        ...
        if (cx < game.map.length) {
          if (
            game.map[cx][musuh.charY][0] == 0 ||
            game.map[cx][musuh.charY][0] == 5
          ) {
            musuh.posX += musuh.sx;
    ...
    if (musuh.stat < 2 && game.aktif) {
      if (
        jarak(game.karakter.x, game.karakter.y, musuh.x, musuh.y) < game.tileW
      ) {
        if (game.lompatY > 0) {
          //injak musuh
          musuh.stat = 2;
          ...
          this.mainkanSuara(dataSuara.bunuh);
        } else {
          heroDead();
        }
      }
